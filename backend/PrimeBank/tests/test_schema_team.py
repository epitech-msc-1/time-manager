...
def make_info_with_admin():
    # Retourne un objet info avec un user admin authentifié
    user = SimpleNamespace(is_authenticated=True, is_admin=True)
    return SimpleNamespace(context=SimpleNamespace(user=user))


def test_create_team_mutation_unit(mocker):
    # On patch la classe Team utilisée dans le module schema_team
    ...
    # Crée un info simulant un user admin authentifié
    ...
    # Après la création, la mutation fixe nr_members = 0 pour la GraphQL type
