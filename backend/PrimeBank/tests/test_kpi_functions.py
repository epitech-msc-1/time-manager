...
def test_determine_team_for_user_and_collect_members():
    ...
    # _collect_team_members should include the manager and member, unique and sorted
    collected = _collect_team_members(team)
    assert len(collected) == 2
    assert collected[0].first_name.lower() <= collected[1].first_name.lower()

    # _determine_team_for_user: if user has managed team, return that
    class UserWithTeam:
...
def test_calculate_presence_score():
    ...
    # 35 hours contract, for period 7 days -> expected 35 hours -> 5 work days
    user = User(35)
    score = _calculate_presence_score(user, period_days=7, days_present=4)
    assert score == 80

    # days_present > denominator should be capped to 100
